...
import { FeedEntityComponentType, FeedEntityStyle, FeedEntityType, FeedRecommendation, IconType, InputType, UserInputStates, UserInputConfiguration, UserInputStyle } from "./type";
...
it('should detect stale local recommandation states', function() {
  ...
  expect(
    getStaleLocalUserInputStates(localRecommandationsState, serverRecommandations).map(({id}) => id)
  ).toEqual([2])
})

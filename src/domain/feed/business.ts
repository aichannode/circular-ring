...
import { FeedEntity, FeedNotification, FeedEntityType, FeedRecommendation, FeedEntityComponentDto, FeedEntityComponentType, UserInputStates, UserInputComponentConfigurationDto } from "./type";
 ...
 */
export function reconciliate(localUserInputStates: UserInputStates): (value: FeedRecommendation, index: number, array: FeedRecommendation[]) => FeedRecommendation {
...
export function removeStaleLocalAnswers(localRecoStates: UserInputStates, serverRecos: FeedRecommendation[], feedStorage: FeedStorage) {
    getStaleLocalUserInputStates(localRecoStates, serverRecos)
        .forEach(({id}) => feedStorage.removeById(id))
}
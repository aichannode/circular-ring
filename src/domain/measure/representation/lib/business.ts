...
export function getSleepStages(
	data: RangeMetrics<MetricType.UserSleepStage, DailySleepStageDuration>,
	isoDay: string
): DailySleepData {
	...
	const usedStageInfos = stageInfos.slice(
		hasCoreSleep
			? stageInfos.findIndex((block) => moment(coreSleepTiming?.[0]).isBetween(block.start, block.end, undefined, "[)"))
		...
		sleepStagesDuration: {
			[SleepStage.AWAKE]:
				...
				MetricType.UserDailyPercAwakeStage in data.fixedValues
					...
					: undefined,
			[SleepStage.REM]:
				MetricType.UserDailyREMStageDuration in data.fixedValues && MetricType.UserDailyPercREMStage in data.fixedValues
					? {

...
export default class Fade extends React.Component<FadeProps, State> {
	...
	constructor(props: FadeProps) {
		...
		const opacityFrom = !!isAnimatedOnMount ? (isVisible ? 0 : 1) : isVisible ? 1 : 0;

		const opacityTo = isAnimatedOnMount ? (isVisible ? 1 : 0) : isVisible ? 0 : 1;
	...
	public render() {
		...
		return <Animated.View style={[style, this.animatedStyle]}>{isVisible && children}</Animated.View>;
	}

...
function getIntersections(
    ...
    for (let i = 0; i < inputSegments.length; i++) {
        ...
        if (stageDuration < sampleSize) {
            ...
            if (
                moment(stage.start).isBetween(from, to)
                && !moment(stage.end).isBetween(from, to)
            ){
                intersections.push({
                    index: i,
                    from: stage.start,
        ...
        // The stage is wider than the sample buffer
            ...
            if (
                moment(from).isBetween(stage.start, stage.end)
                && !moment(to).isBetween(stage.start, stage.end)
            ){
                intersections.push({
                    index: i,
                    from,
...
export function sample<T extends SleepStage | ActivityStage>(
    ...
    for (let i = 0; i < sampleNb; i++) {
        const from = moment(stages[0].start).add(sampleSize * i).valueOf()

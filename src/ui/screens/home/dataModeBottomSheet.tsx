...
export function DataModeBottomSheet({ onClose }: { onClose: () => void }) {
	...
	const save = useCallback(async () => {
		...
		if (!checkbox) appStateService.showDataRatePopup.set(true);
		setLoading(false);
		onClose();
	}, [selectedOption]);
	...
	return (
		<Container horizontalPadding={0}>
			<TopContainer>
				<Title>{format("home.bottomsheet.ring.data.rate.title")}</Title>
				<Description>{format("home.bottomsheet.ring.data.rate.description")}</Description>
			</TopContainer>
			<StyledScrollView>
			...
			<CheckBox
				...
				label={format("home.bottomsheet.ring.data.rate.hide")}
			/>
			<BottomContainer>

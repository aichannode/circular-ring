...
function Header({title, isClosed}: UserInputConfiguration & {isClosed: boolean}) {
    ...
    return (
        <View style={{
            ...
            <Strong style={{flex: 1}}><PrimaryText>{format(title ?? "CR-472 add title") }</PrimaryText></Strong>
...
function Foldable({
    ...
    const nextAnimationDirection = useSharedValue<"closing"|"opening">(isAnimatedOnMount
        ? isClosed ? "closing" : "opening"
        : isClosed ? "opening" : "closing"
    )
    const [isVisible, setIsVisible] = useState(isAnimatedOnMount
        ? isClosed ? true : false 
        : isClosed ? false : true
    )
    const animatedValue = useSharedValue(isAnimatedOnMount
        ? isClosed ? 0 : -1
        : isClosed ? -1 : 0
    );
...
function animate(
    ...
    animatedValue.value = withTiming(nextAnimationDirection.value === "closing" ? -1 : 0, {
        duration: 200,

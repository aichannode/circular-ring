import { Banner, BannerStyle, Activity, BannerComponentType, UserInputComponentConfigurationDto } from "@domain/homeBanner/homeBanner";
...
export const ActivityBanner: React.FC<Props> = ({ banner }) => {
	const highlightColor = getColorFromBannerStyle(banner.style)
	const userSettings = useUserSettings()
	const todayIso = new Date().toISOString();
	const { format } = useI18n();
	const userInput = banner.components.find(({type}) => type === BannerComponentType.USER_INPUT) as UserInputComponentConfigurationDto | undefined

...
export function Hypnogram({data}: Props) {
    ...
    return (        
        <VictoryChart style={{ parent: { maxWidth: "50%" } }}>
            <VictoryAxis
                tickFormat={(tick) => moment(tick).format("H A")}
                style={axisXStyle}
            />
            <VictoryAxis
                domain={yDomain}
                ...
                tickFormat={function(tick) {
                    switch(tick){
                        case SleepStage.DEEP: return format("sleep.stage.deep")
                        case SleepStage.LIGHT: return format("sleep.stage.light")
                        case SleepStage.REM: return format("sleep.stage.REM")
                        default:
                        case SleepStage.AWAKE: return format("sleep.stage.awake")
                    }
                }}
            />
            <VictoryArea
